...
def task3():
    ...
    query = """
SELECT info ->> 'nationality' AS nationality, COUNT(*) AS nat_count
FROM moma_artists
WHERE info ->> 'nationality' IS NOT NULL
GROUP BY nationality
ORDER BY nat_count DESC
LIMIT 10;
    ...
    xpos = np.arange(len(dataframe))
    axes.bar(xpos, dataframe["nat_count"], width=0.50)
    axes.set_xticks(xpos)
...
def task4():
    ...
    query = """
    SELECT UPPER(info ->> 'gender') AS gender, COUNT(*) AS gen_count
    FROM moma_artists
...
def task5():
    ...
    title = "Accumulated Works of Art"
    query = """
