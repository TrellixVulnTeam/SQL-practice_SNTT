...
FROM products p
WHERE p.discontinued = false AND
...
SELECT o.ship_address, COUNT(o.order_id) AS order_count
FROM orders o
...
WITH shippers_per_customer AS (
    SELECT COUNT(DISTINCT o.ship_via) AS shipper_count
    FROM orders o
    GROUP BY o.customer_id
)
SELECT AVG(shipper_count) AS average_shippers_per_customer
